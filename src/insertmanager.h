...
public:
    ...
    explicit InsertManager(const TableData::Pointer &table);
    explicit InsertManager(const Table &table);

    InsertManager &into(const Table &table);
    InsertManager &into(const TableNode::Pointer &table);
    ...
    InsertManager select(const SelectStatement::Pointer &select);
